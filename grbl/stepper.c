...
ISR(TIMER1_COMPA_vect)
{
  ...
  // Then pulse the stepping pins
      ...
      /*TODO
      st.step_bits = 0;
      st.step_bits = (STEP_PORT(0) & ~X_STEP_MASK) | (st.step_outbits & X_STEP_MASK); // Store out_bits to prevent overwriting.
      ...
      */
    #else
      STEP_PORT(0) = (STEP_PORT(0) & ~(X_STEP_MASK | Y_STEP_MASK)) | (st.step_outbits & (X_STEP_MASK | Y_STEP_MASK));
      STEP_PORT(2) = (STEP_PORT(2) & ~(Z_STEP_MASK)) | (st.step_outbits & Z_STEP_MASK);
    #endif
  #else  
    #ifdef STEP_PULSE_DELAY
  ...
  // Reset step out bits.
  st.step_outbits = 0;		//to tu musi zostac, wyzej kod jest wiekszy i minimalnie wolniejszy
...
step_off:
  #ifdef DEFAULTS_RAMPS_BOARD
    STEP_PORT(0) = (STEP_PORT(0) & ~(X_STEP_MASK | Y_STEP_MASK)) | (step_port_invert_mask & (X_STEP_MASK | Y_STEP_MASK));
    STEP_PORT(2) = (STEP_PORT(2) & ~(Z_STEP_MASK)) | (step_port_invert_mask & Z_STEP_MASK);
  #else
...
void st_reset()
{
  ...
  st_generate_step_dir_invert_masks();
    ...
    DIRECTION_PORT(2) = (DIRECTION_PORT(2) & ~(1 << DIRECTION_BIT(2))) | (((dir_port_invert_mask & Z_DIRECTION_MASK) >> Z_DIRECTION_BIT) << DIRECTION_BIT(2));
